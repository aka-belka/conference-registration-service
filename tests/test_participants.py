...
def test_participant_creation():
    p = Participant(1, "Бельская Валерия", "sergeevnau18@gmail.com")
    assert p.id == 1
    assert p.name == "Бельская Валерия"
    assert p.email == "sergeevnau18@gmail.com"
    assert "Бельская Валерия" in str(p)
...
def test_find_participant_by_email():
    ...
    assert find_participant_by_email(participants, "none@mail.com") is None
